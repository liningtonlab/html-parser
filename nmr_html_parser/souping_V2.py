...
def compound_number(compounds, headers):
    ...
    elif any("Î´C" or "Î´H" in s for s in
             headers):  # 3;  Change to regex pattern (Could have spaces, H/C in one, different characters)
        search = ["Î´H", "Î´C"]
    ...
    for item in d2_list:
        ...
        for value in item:
            if CNMR_pattern_1.search(value):  # if CNMR_pattern_1 found with .search regex:
                ...
                Carbon_spec1.append(CNMR_pattern_1.sub("",
                                                       value))  # while removing from original by adding everything but pattern to new list
