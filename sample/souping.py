...
## STRATEGY
    ...
    #   - C + H, C, H
    #   - User defined (function arguement)
    #   - Auto-detect
...
def soup_id_headers(soup):
    ...
    header_1 = [cell_clean(i) for i in thead.find_all("th", align="center", class_="colsep0 rowsep0")]
...
def soup_comp_id(soup):
    thead = soup.find("table")
    header_1 = [cell_clean(i) for i in thead.find_all("th", align="center", class_="rowsep1 colsep0")]
    print(header_1)
    return header_1
        
def soup_id_rows(soup):
    rows = [[cell_clean(j) for j in i.find_all("td")] for i in soup.find("table").tbody.find_all("tr")]
    print(rows)
    return rows

def get_columns(rows, header_2):
    # TODO: Have it working in main file, but getting header_2 to be called and print out like in num_columns
    columns = [[x[j] for x in rows] for j in range(len(header_2))]
    print(columns)
    return columns
